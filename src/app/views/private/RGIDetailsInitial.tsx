  ...
  const send = async () => {
    try {
      for (const element of nfs) {
        ...
        // Garante que cardData e cardData.id estejam definidos antes de chamar a função
        if (cardData && cardData.id) {
          await updateGarantiaItemByIdAsync(cardData.id, item);
        } else {
          console.error("cardData ou cardData.id está indefinido.");
        }
      }
    } catch (error) {
      console.error("Erro ao atualizar item da garantia:", error);
    }
  };
        ...
        {nfs.map((nf, index) => (
          <div key={index} className={styles.nfsItem}>
            <div style={{ display: "flex", alignItems: "center" }}>
              <FileOutlined
                style={{ marginRight: "10px", marginLeft: "20px", fontSize: "20px", color: "red" }}
              />
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
              ...
              <Button
                ...
                onClick={() => navigate(`/garantias/rgi/details-itens-nf/${id}`)}
              >
                &gt;
              </Button>
            </div>
