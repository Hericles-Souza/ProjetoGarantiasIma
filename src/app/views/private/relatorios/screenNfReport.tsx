...
import { getAllNewItems } from "@shared/services/GarantiasService";
// Substitua 'getAllNewItems' pelo nome do serviço correspondente ao novo endpoint
...
const NfReportScreen: React.FC = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            setLoading(true);
            try {
                const response = await getAllNewItems();
                ...
                message.error("Erro ao buscar itens: " + (error.message || "Erro desconhecido"));
                setData([]);
                setFilteredData([]);
                setColumns([]);
                setSelectedColumns([]);
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Relatorio_Novo");
        XLSX.writeFile(workbook, "relatorio_novo.xlsx");
    };
