  ...
  useEffect(() => {
    const fetchGarantias = async () => {
      ...
      try {
        ...
        const dynamicColumns: TableColumnsType<ReportData> = Object.keys(
          firstItem
        )
        ...
        setSelectedColumns(
          Object.keys(firstItem).filter((key) => !excludedKeys.includes(key))
        );
        ...
        const uniqueStatuses = [
          ...new Set(
            garantias.map((item) => item[statusField]).filter(Boolean)
          ),
        ];
        ...
        message.error(
          "Erro ao carregar as garantias. Tente novamente mais tarde."
        );
      } finally {
        setLoading(false);
      }
  ...
  const applyFilters = () => {
    ...
    if (filters.status && filters.statusField) {
      result = result.filter(
        (item) => item[filters.statusField] === filters.status
      );
    }
          ...
          <Card className="shadow-lg">
            ...
            <div style={{ overflowX: "auto" }}>
              <Table
                ...
                loading={loading}
                rowKey={(record) =>
                  record.id || record._id || Math.random().toString()
                }
                pagination={{
                  current: currentPage,
                  pageSize: pageSize,
