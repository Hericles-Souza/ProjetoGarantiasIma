...
import { getAllAcordos } from "@shared/services/AcordoComercialService";
...
const AcordosReportScreen: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAcordos = async () => {
            setLoading(true);
            try {
                const response = await getAllAcordos(currentPage, pageSize);
                const acordos: ReportData[] = response?.data?.data?.data || [];
                const total = response?.data?.data?.total || acordos.length;
                ...
                const uniqueStatuses = [
                    ...new Set(acordos.map((item) => item[statusField]).filter(Boolean)),
                ];
                setStatusOptions(uniqueStatuses);

                setData(acordos);
                setFilteredData(acordos);
                setTotalItems(total);
        ...
        fetchAcordos();
    }, [currentPage, pageSize, filters.search, filters.status, filters.dateRange]);
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Relatorio_Acordos");
        XLSX.writeFile(workbook, "relatorio_acordos.xlsx");
    };
...
export default AcordosReportScreen;